...
while (choise.Key!=ConsoleKey.Q)
{
    ...
    Console.Clear();
    if (sign_count>=sign) //если в есть цифра под нужным нам номером
    {
    ...
    {
         Console.WriteLine($"Вы ввели число {number} и хотели найти его {sign} цифру, результат: в числе {sign_count} цифр, {sign}ая цифра отсутсвует");
    }
