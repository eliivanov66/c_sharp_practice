...
public class Librarys
{
    ...
    //метод конверсии десятичного числа в другую систему изчисления
    public static int convert_dec_to_base(int arg_input, int arg_base)
    {
        if (arg_base>=2)
        {
            int temp_input=0;
            ...
            position=show_sign_count_int(temp_input)-1;
            while (temp_input!=0)
            {
                result =  ((temp_input % arg_base)) + result; 
                temp_input=temp_input / arg_base;
                position--;
            }
