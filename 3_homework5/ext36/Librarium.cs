public class Librarium
{
 ...
 //метод возвращающий координаты в трёхмерном пространстве из строки
    ...
    public static double power(double arg_number, double arg_power)
    {
        double result=1.0;
        //степень меньше -1
        if ((arg_power<=-1))
        {
            for (int i = 0; i > arg_power; i--)
            {
                if (arg_number!=0) //нет делению на ноль
                {
                    result=(result)*(1/arg_number);
                }
            }
        }
        //степень больше 1
        ...
        //извлечение корня
        if ((arg_power<1.0 && arg_power>0.0) || (arg_power>-1.0 && arg_power<0.0))
        {
            //корень извлекается методом последовательной итерации
            //пока пустота
        }
        //возвездение в 0 степень
        if (arg_power==0.0) 
        {
            result=1.0;
        }
        //возведения в степень 0
        if ((arg_power!=0.0) && (arg_number==0.0))
        {
            result=0.0;
        }
        return result;
    ...
    //метод возвращающий количество знаков в числе
    public static int sum_sign (int input)
    {
        ...
        {
            sum_sign=sum_sign + (temp % Convert.ToInt32(power(10, i))/Convert.ToInt32(power(10.0, i-1)));
        }
        return sum_sign;
    }
