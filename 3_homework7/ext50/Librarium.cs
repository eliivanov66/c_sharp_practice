public class Librarium
{
   ...
   static public void DisplayMatrix(int[,] ArgMatrix)
    ...
    public static (bool, int,int) FindInMatrix(int[,] ArgMatrix, int ArgTarget)
    {
        bool TempResult=default(bool);
        int TempLine=default(int);
        int TempRow=default(int);
        ...
        for (int i = 0; i < ArgMatrix.GetLength(0); i++)
        {
            ...
            for (int j = 0; j < ArgMatrix.GetLength(1); j++)
            {
                TempResult=(ArgMatrix[i,j]==ArgTarget);
                if (TempResult)
                {
                    TempLine=i;
                    TempRow=j;
        ...
        return (TempResult, TempLine, TempRow);
    }
}